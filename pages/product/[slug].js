...
const Slug = ({ error, product, variants }) => {
    ...
    const checkServiceability = async () => {
        ...
        if (Object.keys(pinJson).includes(pin)) {
            ...
            setService(false);
            toast.error('Sorry, we cant deliver at this location', {
                position: "top-right",
                autoClose: 2000,
                hideProgressBar: false,
                closeOnClick: true,
                ...
                <div className="lg:w-4/5 mx-auto flex flex-wrap">
                    ...
                    <div className="lg:w-1/2 w-full lg:pl-10 py-6 mt-6 lg:mt-0">
                        ...
                        <div className="flex mt-6 justify-between">
                            <button disabled={product.availableQty <= 0} onClick={() => buyNow(slug, 1, product.price, product.title, product.size, product.color)} className="flex justify-center w-1/2 mr-10 text-white bg-pink-500 border-0 py-2 focus:outline-none disabled:bg-pink-300 hover:bg-pink-600 rounded">Buy Now</button>
                            <button disabled={product.availableQty <= 0} onClick={() => addToCart(slug, 1, product.price, product.title, product.size, product.color)} className="flex justify-center w-1/2 text-white bg-pink-500 border-0 py-2 focus:outline-none disabled:bg-pink-300 hover:bg-pink-600 rounded">Add to Cart</button>
                        </div>
